...
//
static std::map<std::tuple<int,int,int,int>, HFONT> font_list;
static void set_font(WindowData* pWndData)
{
    ...
    {
	xscale = double(pWndData->t_scale[0]) / pWndData->t_scale[1];
	yscale = double(pWndData->t_scale[2]) / pWndData->t_scale[3];
    ...
    auto font_info=std::make_tuple(pWndData->textInfo.font,
       int(font_metrics[pWndData->textInfo.font][mindex].height * yscale),
       int(font_metrics[pWndData->textInfo.font][mindex].width  * xscale),
       pWndData->textInfo.direction);
    if(font_list.find(font_info)==font_list.end())
    {
      hFont = CreateFont(
  	std::get<1>(font_info), 
  	std::get<2>(font_info),
  	std::get<3>(font_info) * 900,
  	(std::get<3>(font_info) & 1) * 900,
  	font_weight[std::get<0>(font_info)],
  	FALSE,
  	FALSE,
  	FALSE,
  	DEFAULT_CHARSET,
  	...
  	CLIP_DEFAULT_PRECIS,
  	DEFAULT_QUALITY,
  	font_family[std::get<0>(font_info)],
  	font_name[std::get<0>(font_info)]
  	);
       font_list[font_info]=hFont;
     }
     else
       hFont=font_list[font_info];
