...
void graphdefaults( )
{
    ...
    */
    ...
    BGI__Colors[0] =BGI__Colors[16]= RGB( 0, 0, 0 );         // Black
...
( int width, int height, const char* title, int left, int top, bool dbflag , bool closeflag)
{
    ...
    HWND con_win=GetConsoleWindow();

    if(con_win && !IsIconic(con_win)) ShowWindow(con_win, SW_MINIMIZE);
...
int _bgi_getch(int special)
{
    ...
    // end critical section
      ...
      if(c==WINDOW_CLOSE) break;
      if(c&MOUSE_LCLICK) pWndData->nmouse_key--;
    } while((special&2)==0 && (c&MOUSE_LCLICK)!=0);
    if(c==0 && special)
    { c = KEY_SPECIAL+_bgi_getch();
    }   
    return c;
}
...
{ 
  ...
  ShowWindow(con_win, SW_SHOWNORMAL);
}
...
int _bgi_kbhit( int mousehit)
{
    ...
    return mousehit?pWndData->kbd_queue.size( )>0:(pWndData->kbd_queue.size( )>pWndData->nmouse_key);
}
