...
{
    return ( RGB_COLOR_BIT | RGB(r,g,b));
}
...
void setbkcolor( int color )
{
    ...
    for ( int i = 0; i < MAX_PAGES; i++ )
    {
      if(pWndData->bgColor==NO_COLOR)
        SetBkMode( pWndData->hDC[i], TRANSPARENT );
      else
      {
        SetBkMode( pWndData->hDC[i], OPAQUE );
...
//
void setfillstyle( int pattern, int color, int bkcolor)
{
    WindowData* pWndData = BGI__GetWindowDataPtr( );
    HDC hDC = BGI__GetWinbgiDC( );
    HBRUSH hBrush;
    ...
    unsigned short Slash[8]      = { us(~0xE0), us(~0xC1), us(~0x83), us(~0x07), us(~0x0E), us(~0x1C), us(~0x38), us(~0x70) };
    unsigned short BkSlash[8]    = { us(~0x07), us(~0x83), us(~0xC1), us(~0xE0), us(~0x70), us(~0x38), us(~0x1C), us(~0x0E) };
    unsigned short Interleave[8] = { us(~0xCC), us(~0x33), us(~0xCC), us(~0x33), us(~0xCC), us(~0x33), us(~0xCC), us(~0x33) };
    unsigned short WideDot[8]    = { us(~0x80), us(~0x00), us(~0x08), us(~0x00), us(~0x80), us(~0x00), us(~0x08), us(~0x00) };
    unsigned short CloseDot[8]   = { us(~0x88), us(~0x00), us(~0x22), us(~0x00), us(~0x88), us(~0x00), us(~0x22), us(~0x00) };
    HBITMAP hBitmap;
    ...
    if(bkcolor!=-1)
    {
      pWndData->bgColor = bkcolor;

      if(bkcolor==NO_COLOR)
        SetBkMode( hDC, TRANSPARENT );
      else
      {
        SetBkColor( hDC, converttorgb(bkcolor) );
